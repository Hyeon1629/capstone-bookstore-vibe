...
STEP 4  ─ 책방 상세 (F-04)
STEP 5  ─ GPS 방문 인증 + 분위기 입력 (F-05 + F-06) + 시연 모드
...
[반드시 포함 (O)]
- 프로젝트의 명확한 목적 (한 줄)
- 핵심 제약 (학교 과제, Capacitor, Firebase, 로컬 시연 등)
- 빌드/실행/테스트 CLI 명령어
- 이 프로젝트만의 독특한 아키텍처 결정 (Mock GPS 시연 모드, 책방 시드 데이터 위치 등)
...
[강조 규칙]
- 어겼을 때 빌드 실패 / 데모 깨짐 / 데이터 손상 가능성이 있는 항목에만 "IMPORTANT" 또는 "YOU MUST" 사용
- 전체 문서에서 5건 이내로 제한
...
- YOU MUST 책방 시드 데이터(15~20곳)는 src/data/bookstores.ts 에 하드코딩한다. 외부 API에서 fetch 하지 않는다.
...
visits
  ...
  bookstoreId      string (bookstores.ts의 id 참조)
  visitedAt        timestamp
...
## 아키텍처 결정 노트
- 책방 시드 데이터는 외부 API 의존성을 만들지 않기 위해 코드에 박는다. 추후 확장 시 Firestore bookstores 컬렉션으로 이전 가능.
- 시연 모드: 마이페이지 → "시연 모드 ON" 토글 시, 지도 핀을 long-press 하면 해당 책방 좌표로 GPS를 임시 mock하고 방문 인증 트리거. 시연/발표용 전용 기능.
...
## 데이터 흐름 단일 진실
- 책방 정적 정보: src/data/bookstores.ts
...
src/
  components/
    ...
    shared/            BottomNav, PhoneFrame(데모용)
...
카카오 맵 + 책방 시드 데이터 + SVG 핀 + 카테고리 토글 + 책방 이름 검색 + 핀 미리보기 시트.
...
2. 책방 시드 데이터 (src/data/bookstores.ts)
   - 총 17곳 (마포 8 + 성동 5 + 종로 4)
   - 마포구 내 신수동/대흥동 일대에 다음 두 곳 반드시 실제 정보로 포함:
     ・ 마포구립서강도서관 — 서울 마포구 독막로 165, 약 37.5485 / 126.9430, 02-3141-7053
     ・ 서강대학교 로욜라도서관 — 서울 마포구 백범로 35 (신수동), 약 37.5510 / 126.9415
   - 나머지 15곳은 실제 좌표 + 가공 이름 (예: "성수 책방", "독립 책방 A")
   - TypeScript 인터페이스 Bookstore 정의 필수
   - 필드: id, name, category('bookstore'|'library'), address, dong, lat, lng,
     hours, phone, specialtyTags, photos(placeholder 경로 배열, 2~3장)
   - 5~7곳에는 미리 시드 분위기 태그 데이터 (이후 STEP 5에서 Firestore에 일괄 import 또는 코드 내장 mock 데이터로 처리)
...
# 2) 시드 데이터 검증
node -e "
const { bookstores } = require('./src/data/bookstores.ts'); // ts-node 또는 tsx 필요
const seoulMapo = bookstores.filter(b => b.address.includes('마포구'));
console.assert(seoulMapo.length >= 5, '마포구 책방 5곳 이상이어야 함');
const seogang = bookstores.find(b => b.name.includes('마포구립서강도서관'));
console.assert(seogang, '마포구립서강도서관 포함 필수');
const sogang = bookstores.find(b => b.name.includes('서강대학교 로욜라'));
console.assert(sogang, '서강대 로욜라도서관 포함 필수');
console.assert(bookstores.length >= 15 && bookstores.length <= 20, '총 15~20곳');
...
# 6) 수동 시연 시나리오 (README_TEST.md 에 작성)
# - /map 진입 시 17개 핀 모두 표시
# - 카테고리 토글 → 해당 색 핀만 표시
# - "성수" 검색 → 성수 책방만 강조
# - 핀 탭 → 하단 시트 슬라이드 업
...
[STEP 3 완료 보고]
- 카카오 맵 통합: ✅
- 책방 시드: ✅ (17곳, 마포구립서강도서관/서강대 로욜라 포함)
- SVG 핀: ✅ (2색 카테고리 + 골드 배지 + 펜던트 글로우)
- 카테고리 토글: ✅
- 검색바: ✅ (부분 일치 핀 강조)
- 미리보기 시트: ✅
- 자가 검증: 6/6 통과
- 권장 커밋 메시지: "feat(map): 지도 홈 + 카카오 맵 + 17개 책방 핀 + 필터·검색"
- 다음 단계: STEP 4 (책방 상세)
```
...
1. 라우트
   - /bookstore/:id (useParams로 id 추출 → bookstores.ts 에서 찾기, 없으면 404 페이지)

2. 화면 구성 (위→아래)
   - 사진 슬라이드 (좌우 스와이프, 도트 인디케이터, 크로스페이드 0.3s)
   - 기본 정보 카드 (이름, 카테고리, 영업 상태(현재 시각 기반 계산), 운영시간, 주소, 거리(현재 위치 기반 haversine))
...
6. 영업 상태 계산 (src/lib/businessHours.ts)
   - bookstore.hours (예: "09:00-22:00" 또는 "평일 09:00-22:00, 주말 09:00-20:00") 파싱
   - 현재 요일·시각으로 영업중/영업종료/곧 종료 계산
   - 휴무일 처리
...
# 3) businessHours 파서 단위 테스트
# Claude Code는 다음 케이스를 src/lib/businessHours.test.ts 에 자동 작성:
# - "09:00-22:00" + 현재 15시 → '영업중'
# - "09:00-22:00" + 현재 23시 → '영업종료'
# - "09:00-22:00" + 현재 21:45 → '영업중 (15분 후 종료)'
# - "평일 09:00-22:00, 주말 09:00-20:00" + 토요일 19시 → '영업중'
# - 휴무일 화요일 + 화요일 → '휴무'
npm test -- businessHours
# → all pass

# 4) TanStack Query 의존성 사용
grep "useQuery" src/hooks/useMoodTags.ts
# → 1건 이상

# 5) 외부 연결 URL scheme 사용 (tel:, map.kakao.com)
...
- 외부 연결: ✅ (전화 / 길찾기 / 공유)
- 자가 검증: 5/5
- 권장 커밋 메시지: "feat(detail): 책방 상세 페이지 + 영업 상태 + 분위기 집계"
- 다음 단계: STEP 5 (GPS 인증 + 분위기 입력 + 시연 모드)
```

---

# STEP 5 — GPS 방문 인증 + 분위기 입력 + 시연 모드 (F-05 + F-06)

GPS 자동 인증, 분위기 모달, 시연 모드 토글 (학교 발표용 핵심 기능).
...
```text
[STEP 5] GPS 방문 인증, 분위기 입력 모달, 시연 모드를 구현한다.

[Plan 단계]
docs/STEP5_visit_mood_demo.md 를 작성하고 승인을 받는다. 포함 항목:

1. GPS 인증 트리거 조건
   - 위치 허용 + 책방 좌표 50m 이내 + 5초 체류 (시연용으로 5분 → 5초 단축)
   - 또는 시연 모드 활성 시 핀 long-press → 즉시 트리거
...
4. 시연 모드 (src/stores/demoStore.ts)
   - isDemoMode: boolean (마이페이지 토글)
   - mockUserLocation: { lat, lng } | null
   - 시연 모드 ON 시:
     ・ 지도 우상단에 작은 "시연 모드" 배지 표시
     ・ 핀 long-press(500ms) → 해당 책방 좌표로 mockUserLocation 설정 → 즉시 GPS 인증 트리거
     ・ 50m 거리 검사 우회

5. GPS 위치 추적 훅 (src/hooks/useGeolocation.ts)
   - navigator.geolocation.watchPosition (앱 활성 상태에서만)
   - 시연 모드 시 mockUserLocation 우선 반환
   - 30초 throttle (배터리 절약)

6. 거리 기반 인증 트리거 (src/hooks/useVisitAutoDetect.ts)
...
9. 수정/생성될 파일 목록
   ...
   - src/pages/map/index.tsx (long-press 핸들러 추가)
...
# 3) 시연 모드 store 정의
grep -E "isDemoMode|mockUserLocation" src/stores/demoStore.ts | wc -l
# → 최소 2
...
# - 시연 모드 OFF: 책방 근처 가지 않으면 인증 안됨
# - 시연 모드 ON: 핀 long-press → 즉시 모달 + Firestore visits 1건 추가
...
[STEP 5 완료 보고]
- GPS 자동 인증: ✅ (50m + 5초)
- 시연 모드: ✅ (long-press 즉시 인증)
- 분위기 모달: ✅ (5 이모지 + 황금 스탬프 + 스파클)
- Firestore 연동: ✅ (visits + moodTags)
- 24시간 중복 방지: ✅
- 자가 검증: 6/6
- 권장 커밋 메시지: "feat(visit): GPS 자동 인증 + 분위기 모달 + 시연 모드"
...
방문 지도, 방문 기록 리스트, 통계, 프로필, 로그아웃, 시연 모드 토글.
...
# 4) 시연 모드 토글이 demoStore와 연결되어 있는가
grep "demoStore\|useDemoStore" src/components/DemoModeToggle.tsx
# → 1건 이상
...
# 7) 수동 시나리오 (README_TEST.md)
# - 신규 가입 직후 → 빈 책장 표시 + CTA
# - 시연 모드로 책방 방문 인증 → 마이 북쉘프 갱신 (실시간)
# - 마이페이지 → 시연 모드 토글 → 지도 화면에 배지 표시
# - 로그아웃 → /login
```
...
- 마이 북쉘프: ✅ (통계 + 방문 지도 + 리스트)
- 빈 상태: ✅ (빈 책장 SVG)
- 마이페이지: ✅ (프로필 + 시연 모드 + 로그아웃)
- 자가 검증: 7/7
- 권장 커밋 메시지: "feat(bookshelf): 마이 북쉘프 + 마이페이지 + 시연 모드 토글"
...
2. capacitor.config.ts
   ...
   - appName: "숨은 책방"
   - webDir: dist
   - server: 개발용 livereload 옵션 (옵션, 시연용)
...
# - 회원가입 → 지도 표시 → 시연 모드로 핀 long-press → 인증 모달
```
...
1. **앱 첫 진입** — 온보딩 슬라이드 2~3장 (Phase 3 핸드오프의 `fadeUp staggered` 시연)
2. **회원가입** — 실시간 검증 보여주기 (특히 닉네임 중복 검증)
3. **지도 홈 진입** — 핀 staggered reveal 영상으로 와우 모먼트 (서울 마포구·신수동 일대)
4. **카테고리 필터 토글** — 주황 ↔ 초록 핀 전환
5. **검색** — "서강" 검색 → 마포구립서강도서관 + 서강대 로욜라도서관 강조
6. **핀 탭** — 하단 미리보기 시트 슬라이드 업
7. **책방 상세** — 분위기 태그 집계, 외부 연결 액션
8. **시연 모드 활성화** — 마이페이지 → 토글
9. **GPS 인증 시뮬레이션** — 지도로 돌아가 핀 long-press → 황금 스탬프 모달 → 분위기 이모지 선택
10. **마이 북쉘프** — 누적 방문 수 카운트 업, 방문 지도 갱신
...
*문서 끝. 본 가이드를 따라 STEP 0부터 순서대로 진행하면 시연 가능한 MVP가 완성됩니다.*
